...
if($row1 && $row1["courseCode1"] == $course1){
    $update1="UPDATE enrolledin SET courseCode1= ' ' WHERE studentID=$id ";
    $updated1=mysqli_query($database, $update1);
    echo "$course1 has been deleted";

} else if($row2 && $row2["courseCode2"]==$course1){
    $update2="UPDATE enrolledin SET courseCode2= ' ' WHERE studentID=$id";
    $updated2=mysqli_query($database, $update2);
    echo "$course1 has been deleted";

} else if($row3 && $row3["courseCode3"]==$course1){
    $update3 = "UPDATE enrolledin SET courseCode3= ' ' WHERE studentID=$id";
    $updated3 = mysqli_query($database, $update3);
    echo "$course1 has been deleted";

} else if($row4 && $row4["courseCode4"]==$course1){
    $update4 = "UPDATE enrolledin SET courseCode4= ' ' WHERE studentID=$id ";
    $updated4 = mysqli_query($database, $update4);
    echo "$course1 has been deleted";

} else if($row5 && $row5["courseCode5"]== $course1){
    $update5 = "UPDATE enrolledin SET courseCode5= ' ' WHERE studentID=$id ";
    $updated5 = mysqli_query($database, $update5);
    echo "$course1 has been deleted";

}else {
    echo "You have no more registered courses";
}
